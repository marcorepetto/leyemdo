...
* **1.1. ¿Qué librería de Python prefieres utilizar para extraer el texto de los PDFs?**
  * [ ] `PyMuPDF` (`fitz`): (Recomendada) Extremadamente rápida y proporciona coordenadas precisas de las palabras, lo cual es fundamental para futuros resaltados visuales en el visor.
...
* **1.2. ¿Se debe realizar algún tipo de limpieza durante el parsing?**
  ...
  * [ ] Sí, limpieza básica (quitar dobles espacios, normalizar guiones y saltos de línea).
...
* **2.1. ¿Qué estrategia de segmentación utilizaremos para dividir el texto?**
  * [ ] **Recursive Character Splitter (Recomendado):** Divide el texto buscando saltos de párrafo, luego oraciones y palabras, intentando mantener los fragmentos en un tamaño de token/caracter objetivo con un solapamiento (overlap).
...
* **3.1. Además de la página del PDF, ¿qué metadatos debemos adjuntar a cada chunk?**
  * [ ] `document_id` (para asociarlo al archivo).
  * [ ] `page_number` (crítico para las citas clicables).
  * [ ] `char_start` / `char_end` (índices de caracteres en el documento original).
  * [ ] `header` / `sección` (si es posible detectarlos con expresiones regulares simples).
...
* **4.1. ¿Cómo debe manejar el backend la carga y procesamiento de archivos grandes?**
  ...
  * [ ] **Asíncrono con FastAPI BackgroundTasks (Recomendado):** El cliente sube el archivo, la API guarda el archivo temporalmente, responde de inmediato con `202 Accepted` y procesa el documento en segundo plano sin bloquear.
