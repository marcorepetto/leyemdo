...
* **Visualización de Cajas Delimitadoras:** Dibujo de rectángulos coloreados sobre cada bloque de texto obtenido de `page.get_text("blocks")` utilizando la API de dibujo nativa de PyMuPDF.
...
* **Número de Ordenación Secuencial:** Dibujo de un número entero en el centro del bloque (con un círculo de fondo semi-transparente para legibilidad) que indique su posición en la secuencia devuelta por `sort_blocks_by_columns`.
* **Interfaz Web Autónoma Embebida:** Página HTML simple servida por FastAPI en el endpoint `/api/v1/debug/ui` que contenga:
  * Input de subida de archivos PDF.
  * Selector de página (anterior, siguiente, número de página).
  * Panel de visualización de la imagen anotada de la página.
* **Endpoints de Depuración:**
  * `POST /api/v1/debug/upload`: Subida temporal de un PDF.
  * `GET /api/v1/debug/view/{document_id}/page/{page_number}`: Renderiza la página del PDF, dibuja las cajas de bloques, las coordenadas de esquinas y los números de orden directos con PyMuPDF, y retorna la imagen en formato PNG (`image/png`).
  * `GET /api/v1/debug/ui`: Retorna la página HTML estática responsiva (HTML/JS embebido).

### Qué NO HACE la Spec (Out-of-Scope)
* **No altera los PDFs originales:** Las anotaciones y dibujos se hacen sobre una copia en memoria y el renderizado PNG temporal, sin modificar el archivo original.
* **No usa Okular:** La visualización es 100% web local y autocontenida.
* **No se conecta con base de datos vectorial:** Se apoya en la estructura temporal de subida en memoria.
...
sequenceDiagram
    ...
    API->>API: Guardar PDF en caché temporal de memoria
    API-->>UI: 200 OK { "document_id": "...", "pages_count": N }
    
    Note over UI, API: El navegador carga la UI y solicita la página 1
    ...
    API->>API: Dibujar rectángulos (rojos), coordenadas (azules) y orden (círculo+número) con PyMuPDF
...
2. Se iniciará la redacción del **Plan Técnico** (archivos exactos a crear y modificar, llamadas de dibujo de PyMuPDF, etc.).
